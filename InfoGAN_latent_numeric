...
build_generator <- function(){
  ...
  cnn %>%
    layer_dense(128*7*7, activation = "relu",input_shape = noise_shape+1) %>%
    ...
    noise <- layer_input(shape = noise_shape+1)
    img <- cnn(noise)
...
adam_beta_1 <- 0.5
c_given_x<-1
infoloss<-function(c,c_given_x){
    eps <- 1e-9
  conditional_entropy = k_mean(- k_sum(k_log(c_given_x + eps) * c, axis=1))
  entropy = k_mean(- k_sum(k_log(c + eps) * c, axis=1))
...
discriminator %>% compile(
  ...
  loss = list("binary_crossentropy",infoloss),metrics='accuracy'
)
...
noise <- layer_input(shape = list(noise_shape+1))
...
for(epoch in 1:epochs){
  ...
  for(index in sambat){
    ...
    sampled_labels <- runif(batch_size,-2,2)
  ...
  sampled_labels <- seq(-2,2,by=0.3)
  ls<-length(sampled_labels)
  noise <- runif(n = noise_shape * ls, min = -1, max = 1)  %>% matrix(nrow=ls)
  noise<-matrix(rep(noise[1,],ls),nrow=ls,byrow=T)
